...
describe("VE403-13", () => {
    ...
    it("VE403-13: Server supports Energistics protocol", async (): Promise<any> => {
        ...
        await httpTest
            ...
            .expect(200)
            .then(response => {
                let supportedProtocols = response.body as Array<string>;
                expect(supportedProtocols.includes("etp12.energistics.org")).toBeTruthy()
            })
...
describe("VE403-18", () => {
    it("VE403-18: Server accepts valid protocols", async () => {
        ...
        await client.connect(config.etpServerUrl, 'etp12.energistics.org', undefined, {
            'Authorization': `${config.jwtToken}`
        });
    });

    it("VE403-18: Server rejects invalid protocols", async () => {
        const WebSocketClient = require('websocket').client;
        ...
        await client.connect(config.etpServerUrl, 'fake', undefined, {
            'Authorization': `${config.jwtToken}`
        });
    });
