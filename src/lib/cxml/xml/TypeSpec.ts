...
import { Member, Rule, RuleClass } from "../parser/Rule";
...
function defineSubstitute(substitute: MemberSpec, proxy: MemberRef) {
  return MemberRef.parseSpec(
    [substitute, 0, substitute.safeName],
    substitute.namespace as any,
    proxy
  );
}
...
export class TypeSpec extends Item {
  constructor(name?: string, namespace?: Namespace, spec?: RawTypeSpec) {
    super(TypeSpec, spec ? spec[1] : undefined);
  ...
  init() {
    ...
    const dependency = this.dependency as TypeSpec;
    let parent = Member;
    ...
    const instanceProto = this.proto.prototype as Member;
  ...
  private defineMember(ref: MemberRef) {
    let typeSpec: TypeSpec | null =
      ref.member.typeSpecList && ref.member.typeSpecList[0];
    let proxySpec = ref.member.proxySpec;
    ...
    if (typeSpec) {
      const memberType = typeSpec.placeHolder;
      const type = this.proto.prototype as RuleMembers;
  ...
  defineMembers() {
    for (const child of this.childSpecList) {
      const memberRef = MemberRef.parseSpec(child, this.namespace);
      this.addChild(memberRef);
      this.defineMember(memberRef);
    }

    for (const attr of this.attributeSpecList) {
      const attributeRef = MemberRef.parseSpec(attr, this.namespace);
  ...
  addSubstitutes(headRef: MemberRef, proxy: MemberRef) {
    ...
    for (const substitute of headRef.member.proxySpec.getSubstitutes()) {
      if (substitute === headRef.member) {
        this.rule.addChild(headRef);
      } else {
        const substituteRef = defineSubstitute(substitute, proxy);
  ...
  addSubstitute(head: MemberSpec, substitute: MemberSpec) {
    if (this.ready && head.containingTypeList.length) {
      ...
      for (const spec of head.containingTypeList) {
        const ref = defineSubstitute(substitute, spec.proxy);
      ...
      while (head) {
        TypeSpec.addSubstituteToProxy(
          substitute,
          head.proxySpec.proto.prototype
        );
  ...
  cleanPlaceholders(strict?: boolean) {
    const type = this.proto.prototype as RuleMembers;
    let nameList = this.optionalList;
  ...
  private static addSubstitutesToProxy(member: MemberSpec, type: RuleMembers) {
    for (const substitute of member.proxySpec.getSubstitutes()) {
